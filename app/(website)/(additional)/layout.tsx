...
export default async function AdditionalRouteGroupLayout({
  children,
}: {
  children: React.ReactNode
}) {
  const additionalNavigation =
    await reader.singletons.additionalNavigation.read()

  if (!additionalNavigation) notFound()
  ...
  return (
    <div className="relative">
      <nav className="3xl:h-16 3xl:px-[100px] sticky top-0 flex h-14 items-center justify-between bg-white px-5">
        ...
        <button
          data-iclosed-link="marketeq/sales-offer-engine-call"
        ...
        >
          {additionalNavigation.actionable}
        </button>
      </nav>
      {children}
    </div>
